...
- E2E: `pnpm --filter client test:e2e` (Playwright, chromium). Requires the full dev stack up (or it starts it via `webServer`) and Postgres reachable by the server. It first seeds/resets the test user's data (`server/scripts/seed-test-user.ts` deletes that user's documents and any `e2e-*` accounts), then runs ordered projects: unauthenticated auth specs → login setup → single-client authenticated specs (dashboard/editor/sharing) sharing a saved session → multi-client collaboration specs (second browser context) → logout spec last. Order matters because login overwrites and logout nulls the user's single stored refresh token server-side — see `client/playwright.config.ts`. Multi-user tests must create their second context with an explicit empty `storageState` (this Playwright version seeds manual contexts with the project's).
